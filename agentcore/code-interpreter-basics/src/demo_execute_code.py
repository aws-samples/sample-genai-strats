...
CODE_TO_EXECUTE = Path("code_to_execute.py").read_text().strip()

l.info("CODE_INTERPRETER_ID=%s", CODE_INTERPRETER_ID)
l.info("CODE_TO_EXECUTE=\n-----START-----\n%s\n----- END -----", CODE_TO_EXECUTE)
...
def main():
    ...
    l.info(f"> Starting a new Code Interpterer Session...")
    ...
    l.info(f"> Started a new Code Interpreter Session, session_id={session_id}")

    input("\nPress ENTER to send CODE_TO_EXECUTE to the Code Interpreter Session\n")
    ...
    l.info(f"> Session terminated. All done!")
